...
header Kernel
  ...
  functions
    ...
    SerialHandlerFunction (value: int)
    ...
    Handle_Sys_ChMode (fileName: ptr to array of char, newMode: int) returns int
    Handle_Sys_ReadDir (fileDesc: int, userDirEntry: ptr to dirEntry) returns int
  ...
  class ThreadManager
    ...
    fields
      ...
      threadManagerMtx: Mutex
      aThreadIsFree: Condition
      threadsInQueue: int
    methods
      Init ()
      Print ()
  ...
  class FrameManager
    ...
    fields
      ...
      framesNeededQueue: int
      numFramesNeededQueue: List [ Listable ]
 ...
 -----------------------------  FileControlBlock  ---------------------------------
  ...
  class OpenFile
    ...
    fields
      ...
      pipe: ptr to Pipe		    -- pipe for any pipes needed
    methods
      Print ()
 ...
 class SerialDriver
   ...
   methods
     ...
     ReadTerminal (file: ptr to OpenFile, userBuffer: ptr to char,
      	        sizeInBytes: int) returns int
     WriteTerminal (file: ptr to OpenFile, userBuffer: ptr to char,
      	        sizeInBytes: int) returns int
     SerialHandler ()
 endClass
...
----------------------------- Pipe --------------------------------
  ...
  const MAX_PIPE_BUFFER_SIZE = 8192
  ...
  class Pipe
    ...
    fields
        ...
        writer: Condition	-- singal there is a writer?
	numWriters: int
